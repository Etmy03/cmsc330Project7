  ...
  where T:Clone                 // elements implement Clone so have .clone()
{
  let n = r0_slice.len();
    ...
    for y in 0..n{
      ...
      for x in 0..n {
        if (x+y)<n{
          my_row.push(r0_slice[x+y].clone());
        }
        else if y != 0{
          for i in 0..y{
            my_row.push(r0_slice[i].clone());
          }
          break;
        }
      }
...
pub fn count_words(text: &String) -> i32 {
  let mut my_word = Vec::new();
    let mut my_sentence = Vec::new();
    for c in text.chars() {
      if c.is_whitespace() || c == '\\'{
        if !my_word.is_empty(){
          my_sentence.push(my_word.clone());
          my_word = Vec::new();
        }
      }
      else{
